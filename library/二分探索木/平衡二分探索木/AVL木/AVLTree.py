...
class AVLtree:
    ...
    def delete(self, value, c=1):
        if value in self.dict:
            self.dict[value] -= min(self.dict[value], c)  
        else: return False
    ...
    def __height__(self, u):
        if u is None:
            return 0
        return u.h

    def __modHeight__(self, u):
        u.h = 1 + max(self.__height__(u.l),self.__height__(u.r))
    ...
    def __bias__(self, u):
        return self.__height__(u.l)-self.__height__(u.r)
